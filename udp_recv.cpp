...
int main(int argc, char **argv)
{
    ...
    ec = bind(fd, (struct sockaddr*)&sockaddr, sizeof(sockaddr));
    EC_CHECK_RETURN(ec);

    char data[1536];
    int length, data_length = 1536;
    while(true)
    {
        length = recv(fd, (void *)data, data_length, 0);
        if(length < 0)
        {
            if(errno != EAGAIN)
            {
        ...
        cout << "recv data length " << length << endl;
        usleep(10 * 1000);
    }
